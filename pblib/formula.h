...
class FormulaClass {
 ...
 public:
  static PBLib::PBConfig config;
  FormulaClass(int32_t flags, int32_t data, copy_tag)
 ...
 public:
  ...
  static Formula newNeg(Formula const& f) {
    ...
    Formula result =
        (isAtom(f))
            ? std::make_shared<FormulaClass>(f->flags, f->data ^ 1, copy_tag{})
            : std::make_shared<FormulaClass>(f->flags, f->data ^ 1,
                                             f->input_nodes, copy_tag{});
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (auto const& formula : it->second) {
          if (isAND(formula) && formula->input_nodes[0] == f &&
              formula->input_nodes[1] == g) {
            return formula;
          }
        }
      }
  ...
  static Formula newEquiv(Formula const& f, Formula const& g) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (auto const& formula : it->second) {
          if (isEquiv(formula) && formula->input_nodes[0] == f &&
              formula->input_nodes[1] == g) {
            return formula;
          }
        }
  ...
  static Formula newFAs(Formula const& x, Formula const& y, Formula const& c) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (auto const& formula : it->second) {
          if (isFAs(formula) && formula->input_nodes[0] == x &&
              formula->input_nodes[1] == y && formula->input_nodes[2] == c) {
            return formula;
          }
        }
      }
  ...
  static Formula newFAc(Formula const& x, Formula const& y, Formula const& c) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (auto const& formula : it->second) {
          if (isFAc(formula) && formula->input_nodes[0] == x &&
              formula->input_nodes[1] == y && formula->input_nodes[2] == c) {
            return formula;
          }
        }
  ...
  static Formula newMonotonic_ITE(Formula const& s, Formula const& t,
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (auto const& formula : it->second) {
          if (isMonotonicITE(formula) && selector(formula) == s &&
              true_branch(formula) == t && false_branch(formula) == f) {
            return formula;
          }
        }
      }
